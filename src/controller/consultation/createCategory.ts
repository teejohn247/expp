...
const createConsultationCategories = async (req: Request , res: Response): Promise<void> => {
    ...
    try {
        ...
        const { categoryName, image, description } = req.body as CategoryRequestBody;
				...
				const checkCategory = await consultationCareCategories.findOne({ categoryName });
       ...
       const category = new consultationCareCategories({
					categoryName,
					image,
					description
        });
...
export default createConsultationCategories;
