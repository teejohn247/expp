...
const fetchCareCategories = async (req: Request , res: Response): Promise<void> => {

    try {

	 		const { page, limit } = req.query as unknown as QueryRequestBody;

			const products = await selfCareCategories.find()
			.limit(limit * 1)
			.skip((page - 1) * limit)
			.exec();
			...
			const count = await selfCareCategories.find().countDocuments();
...
export default fetchCareCategories;
